...
class Corpus:
    ...
    def stats(self, n=10, stop_words=None):
        """
        Returns a DataFrame with term frequency and document frequency of the top n words.
        Uses get_vocab to get unique words first.
        """
        if stop_words is None:
            stop_words = set()
    
        vocab_list = self.get_vocab(stop_words=stop_words)  # sorted list of unique words
    
        term_counter = Counter()
        doc_counter = Counter()
    
        for doc in self.id2doc.values():
            words = [w for w in self.nettoyer_texte(doc.texte).split() if w in vocab_list]
            term_counter.update(words)
            doc_counter.update(set(words))
    
        freq_df = pd.DataFrame({
            "word": list(term_counter.keys()),
            "term_frequency": list(term_counter.values()),
            "document_frequency": [doc_counter[w] for w in term_counter.keys()]
        })
    ...
    # 1.2 Build TF matrix
    # ----------------------------
    def build_TF_matrix(self, stop_words=None):  
